...
class EcSee(BoxLayout):
    ...
    #     accuracy_result = f"Detection accuracy: {accuracy_percentage:.1f}%"
    ...
    #
    #     detected_allergens = set([occurrence.lower() for occurrence in occurrences])
    #     all_allergens = set([allergen.lower() for allergen in allergens])
    #     correct_allergens = detected_allergens.intersection(all_allergens)
    #     num_correct = len(correct_allergens)
    #     if num_detected != 0:
    #         accuracy = (num_correct / num_detected) * 100
    #     else:
    #         accuracy = 100
    #
    #     allergen_accuracy_result = f"Allergen detection accuracy: {accuracy:.1f}%\n"
    #
