...
class Test(BoxLayout):
    ...
    def get_tokens(self):
        ...
        mwe_tokenizer = MWETokenizer([('kacang', 'tanah'), ('ekstrak', 'malt'), ('kayu', 'manis')])
    ...
    def count_allergens(self, button, widget):
        self.ids.my_button1 = button
        self.ids.txtinput = widget
        self.allergen_count = int(widget.text)
        allergens = [
            "susu",
            "gluten",
            "kedelai",
    ...
    def allergen_accuracy(self, allergens, tokens):
        ...
        num_actual = int(self.allergen_count)

        if num_actual != 0:
            self.accuracy = (num_detected / num_actual) * 100
        else:
            if num_actual == 0:
                self.accuracy = 100
            else:
        ...
        allergen_accuracy_result = f'Allergen detection accuracy: {self.accuracy:.2f}%\n'
